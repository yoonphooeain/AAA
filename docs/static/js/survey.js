document.addEventListener("DOMContentLoaded", () => {
  ...
  const staticStorageKey = "aaa_static_survey_responses";
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      if (isStatic) {
        const existing = JSON.parse(window.localStorage.getItem(staticStorageKey) || "[]");
        existing.push({
          ...payload,
          age: payload.age ? Number(payload.age) : null,
          tangibles: Number(payload.tangibles),
          reliability: Number(payload.reliability),
          responsiveness: Number(payload.responsiveness),
          assurance: Number(payload.assurance),
          empathy: Number(payload.empathy),
          security: Number(payload.security),
          digital_convenience: Number(payload.digital_convenience),
          overall_satisfaction: Number(payload.overall_satisfaction),
          created_at: new Date().toISOString(),
        });
        window.localStorage.setItem(staticStorageKey, JSON.stringify(existing));
        const currentResponses = appText.current_responses || "Current Responses";
        const successPrefix = appText.submit_success || "Survey submitted successfully.";
        status.textContent = `${successPrefix} ${currentResponses}: ${existing.length}.`;
